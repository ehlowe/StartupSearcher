...
def best_companies(cur):
    # I want to select all companies with top 200 cb_rank, all companies with the industry_name from industry_connections table == Automation, all companies with seed funding as last_funding_type
...
def pull_all_automations(cur):
    # make sure there are no duplicatesArtificial Intelligence (AI),Automation
    cur.execute("""SELECT *
    FROM crunchbase
